    ...
    <nav style={{
      ...
      <div style={{ padding: '12px 14px', borderTop: `0.5px solid ${theme.border}`, display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <button onClick={() => setTz(t => t === 'UTC' ? 'local' : 'UTC')} style={{
            fontSize: 10, padding: '2px 8px', borderRadius: 4,
            border: `0.5px solid ${theme.border}`,
            background: 'transparent', color: theme.muted, cursor: 'pointer',
          }} title={tz === 'UTC' ? 'Showing UTC — click for local time' : `Showing local time (${Intl.DateTimeFormat().resolvedOptions().timeZone}) — click for UTC`}>{tz === 'UTC' ? 'UTC → Local' : 'Local → UTC'}</button>
        </div>
